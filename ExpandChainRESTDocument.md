...
The example starts from import required Python modules so user have to import request and json module in the example. User also need json module to parse JSON data in the HTTP response.
...
Before sending HTTP request with a post message to HistoricalChainResolution endpoint, it required Authorization Token from previous steps with JSON payload containing Chain RIC with start and end date the user wan to request. Please find sample JSON payload from section **Sample Request Body**.
